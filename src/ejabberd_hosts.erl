...
start_host(Host) when is_list(Host) ->
	...
	%% Starts the default auth method. Does not make any test for host specific 
	%% auth metods, all have to use the same. 
	ejabberd_auth:start_methods(Host, json),
    ok.
...
stop_host(Host) when is_list(Host) ->
	...
	ejabberd_auth:stop_methods(Host, json).
